      ...
      <el-row class="padding-top-20 table-style-none">
        <el-table :data="cartList" class="width--100" ref="cartList" @selection-change="getSelectList"
          ...
          <el-table-column label="操作">
            <template scope="scope">
              <el-button size="small" type="text" @click="showDelete(scope.row)" class="button-border">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-row>
      ...
      getCartList(){
        ...
        this.$store.dispatch('order_a_shoppingCartList', {params: {clientId: this.clientId}}).then((response) => {
          if (response.data.resultCode == 0) {
            this.cartList = response.data.resultData.cartList;
//            console.log(this.cartList)
            this.$nextTick(function () {
              for (let item in this.cartList) {
                if (this.cartList[item].selected === 1) {
...
//            console.log(this.cartList)
      ...
      itemDelete(item){
        ...
        this.$store.dispatch('order_a_shoppingCartDelete', {params: params}).then((response) => {
          if (response.data.resultCode == 0) {
            this.$store.commit('order_m_removeCartListData', params)
            this.getCartList();
          }
        })
      },
...
//        console.log(this.selectList)
      ...
      selectAll(row){
        if (!!row) {
          if (this.selectList.length == row.length) {
            ...
            this.$refs.cartList.clearSelection();
            row.forEach(row => {
              this.$refs.cartList.toggleRowSelection(row);
            });
          }
        } else {
      ...
      getSettlementInfo(){
        ...
        for (let item in this.selectList) {
          ...
          if (this.selectList[item].isSpecial === 1) {
            hasSpecial = true;
          }
          if (this.selectList[item].isSpecial === 0) {
            hasNormal = true;
          }
        }
        if (hasSpecial === true && hasNormal === true) {
          this.$message.error("特殊积分礼品,只能单独下单");
          return;
        }
