      ...
      <div v-for="item in orderInfo.subOrderList">
        <div style="background-color:#f1f1f1;border:1px solid #e5e4e4;border-bottom:none;" class="clearfix">
          <el-form label-width="80px" :inline="true">
            <div class="v-left">
              <el-form-item label="子订单编号 : " class="margin-bottom-0">{{item.baseInfo.orderNumber}}</el-form-item>
              <el-form-item label="子订单状态 : " class="margin-bottom-0">{{getOrderStatus(item.baseInfo.orderStatus)}}
              </el-form-item>
            </div>
        ...
        <el-table :data="item.detailList" class="width--100" :show-header="false">
  ...
  export default{
    ...
    methods: {
      ...
      showCancelOrder(row,baseInfo){
        return row.returnStatus === 0 && baseInfo.orderStatus === '10'
      },
      goToItemInfo(item){
